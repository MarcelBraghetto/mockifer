...
class MainViewController : BaseViewController, NSTableViewDelegate, NSTextViewDelegate, NSTextFieldDelegate {
    ...
    @IBAction func onGoSelected(_ sender: AnyObject) {
        ...
        if isInternalCheckbox.state == NSControl.StateValue.on {
        ...
        if ((jsonContentField.string.count) > 0) {
    ...
    @IBAction func onInternalToggleSelected(_ sender: NSButton) {
        ...
        if isInternalCheckbox.state == NSControl.StateValue.off {
    ...
    func populateRouteDetails() {
        ...
        if isEditingNewMock {
            ...
            if currentRoute.isInternal {
                isInternalCheckbox.state = NSControl.StateValue.on
                lockPushMockButton()
            } else {
                isInternalCheckbox.state = NSControl.StateValue.off
                unlockPushMockButton()
            }
        }
        ...
        if currentRoute.responseJsonFile.count == 0 {
